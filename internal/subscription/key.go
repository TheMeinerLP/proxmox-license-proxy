...
// KeySockets returns the socket count a PVE key encodes (1, 2, 4 or 8), or 0 for
// a non-PVE key (PBS/PMG carry no socket digit). It reads the digit that sits
// between the product code and the level letter, e.g. "pve2c-..." -> 2.
func KeySockets(key string) int {
	if !strings.HasPrefix(key, "pve") || len(key) < 4 {
		return 0
	}
	switch key[3] {
	case '1':
